...
const rules = reactive<FormRules>({
  typeId: [{ required: true, message: '请选择套餐类型', trigger: 'blur' }],
  ...
  originPrice: [{ required: true, message: '请填写套餐原价格', trigger: 'blur' }],
  price: [{ required: true, message: '请填写套餐优惠后价格', trigger: 'blur' }],
...
async function queryPackageTypeList() {
  try {
    typeList.value = await ApiPackage.queryPackageType().then(res => res.data.rows)
  }
  catch (error) {
    loading.value = false
  }
...
<template>
  <div>
    <page-main >
      <el-form ref="formRef" :inline="true" :model="formInline">
        <el-form-item label="套餐状态" prop="status">
          <el-select v-model="formInline.status" placeholder="请选择套餐状态" clearable style="width: 192px;">
    ...
    <page-main bottomPadding="20px" bottomMargin="0px">
      <el-table v-loading="loading" border :data="tableData" height="52vh" style="width: 100%;" size="large">

        <el-table-column fixed prop="name" label="套餐名称" width="150" />
        <el-table-column fixed prop="typeName" label="套餐类型" width="100"  />
        <el-table-column prop="order" label="排序ID" align="center" />

        <el-table-column prop="originPrice" label="套餐原价" width="100" align="center" />
        <el-table-column prop="price" label="套餐优惠后价格" width="100" align="center" />
        ...
        <el-table-column prop="drawMjCount" label="MJ额度" width="100" />
      ...
      >
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            ...
            <el-form-item label="套餐类型" prop="typeId">
              <el-select  v-model="formPackage.typeId" style="width: 100%">
            ...
            <el-form-item label="套餐价格原价格" prop="originPrice">
              <el-input v-model.number="formPackage.originPrice" placeholder="请填写套餐价格(￥)最多两位小数" type="number" />
            </el-form-item>
            <el-form-item label="套餐优惠后价格" prop="price">
          ...
          <el-col :span="12" >
            ...
            <el-form-item label="MJ绘画积分" prop="drawMjCount">
              <el-input v-model.number="formPackage.drawMjCount" type="number" placeholder="MJ绘画积分" />
            </el-form-item>
