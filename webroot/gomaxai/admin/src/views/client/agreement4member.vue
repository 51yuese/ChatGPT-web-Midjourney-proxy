...
const rules = ref<FormRules>({
  memberAgreementTitle: [
    { required: true, trigger: 'blur', message: '请填写会员协议标题' },
  ],
  memberAgreement: [
    { required: true, trigger: 'blur', message: '请填写会员协议内容' },
  ],
})
const formRef = ref<FormInstance>()

async function queryAllconfig() {
  const res = await apiConfig.queryConfig({ keys: ['memberAgreement', 'memberAgreementTitle'] })
  const { memberAgreement, memberAgreementTitle } = res.data
   Object.assign(formInline, { memberAgreement, memberAgreementTitle })
}
...
<template>
  <div>
    <page-main bottomPadding="20px">
      <el-alert :closable="false" show-icon title="会员协议设置说明" description="这里的会员协议设置是对应用户端的会员协议链接、可以使用MD语法或直接使用Html标签进行发布、您也可以直接粘贴上传图片、作为图床使用、其他地方比如上传商品等地方的图片也可以暂时在此作为临时方案。" type="success" />
    </page-main>
      ...
      <el-form ref="formRef" :rules="rules" :model="formInline" label-width="120px">
        <el-row>
          <el-col :xs="24" :md="20" :lg="15" :xl="10">
            <el-form-item label="协议标题" prop="memberAgreementTitle">
              <el-input v-model="formInline.memberAgreementTitle" :rows="1" placeholder="会员协议标题" clearable />
            </el-form-item>
          </el-col>
...
<!--          </el-col>-->
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="协议内容" prop="memberAgreement">
              <MdEditor v-model="formInline.memberAgreement" style="min-height: 80vh;" :theme="theme" @onChange="onChange" @onUploadImg="onUploadImg" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
