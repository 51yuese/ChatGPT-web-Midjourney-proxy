...
const rules = ref<FormRules>({
  serviceAgreementTitle: [
    { required: true, trigger: 'blur', message: '请填写服务协议标题' },
  ],
  serviceAgreement: [
    { required: true, trigger: 'blur', message: '请填写服务协议内容' },
  ],
})
const formRef = ref<FormInstance>()

async function queryAllconfig() {
  const res = await apiConfig.queryConfig({ keys: ['serviceAgreement', 'serviceAgreementTitle'] })
  const { serviceAgreement, serviceAgreementTitle } = res.data
   Object.assign(formInline, { serviceAgreement, serviceAgreementTitle })
}
...
<template>
  <div>
    <page-main bottomPadding="20px">
      <el-alert :closable="false" show-icon title="服务协议设置说明" description="这里的服务协议设置是对应用户端的服务协议链接、可以使用MD语法或直接使用Html标签进行发布、您也可以直接粘贴上传图片、作为图床使用、其他地方比如上传商品等地方的图片也可以暂时在此作为临时方案。" type="success" />
    </page-main>
      ...
      <el-form ref="formRef" :rules="rules" :model="formInline" label-width="120px">
        <el-row>
          <el-col :xs="24" :md="20" :lg="15" :xl="10">
            <el-form-item label="协议标题" prop="serviceAgreementTitle">
              <el-input v-model="formInline.serviceAgreementTitle" :rows="1" placeholder="服务协议标题" clearable />
            </el-form-item>
          </el-col>
...
<!--          </el-col>-->
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="协议内容" prop="serviceAgreement">
              <MdEditor v-model="formInline.serviceAgreement" style="min-height: 80vh;" :theme="theme" @onChange="onChange" @onUploadImg="onUploadImg" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
