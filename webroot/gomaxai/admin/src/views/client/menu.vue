...
const rules = {
  ...
  menuTipText: [{ required: true, trigger: 'blur', message: '请填写菜单名称提示' },],
...
function handleEditMenu(row: any){
  const { id, menuTipText, menuName, menuIframeUrl, isJump, isShow, menuIcon, menuPath, order, isNeedAuth } = row
...
async function handleEditIcon(row: any){
  console.log('row: ', row);
  const { id, menuIcon: icon, menuName: name, order } = row;
  menuIcon.value = icon
  menuName.value = name
...
async function handleConfirmUpdateIcon(){
  if(!menuIcon.value || !menuName.value || !menuTipOrder.value ) return ElMessage.error('请填写ICON编码');
  const params = {
  ...
  <div>
    ...
    <el-dialog v-model="visible" :close-on-click-modal="false" :title="dialogTitle" width="570" @close="handlerCloseDialog(formRef)">
      <el-form ref="formRef" label-width="140" :rules="rules"  :model="formInline">
        ...
        <el-form-item label="菜单名称" prop="menuName">
          <el-input v-model="formInline.menuName" placeholder="请填写菜单名称" />
        </el-form-item>
    ...
    <el-dialog v-model="visibleIcon" title="修改默认信息" width="570" @close="handlerCloseDialog(formRef)">
      ...
      <div style="display: flex; align-items: center;margin-top: 12px;">
        ...
        <el-input  v-model="menuName" placeholder="请填写要修改的菜单名称" />
      </div>
      <div style="display: flex; align-items: center;margin-top: 12px;">
