...
function handleEditKey(row: any) {
  ...
  const { keyType, modelName, key, appid, secret, status, model, keyWeight, canUpload, canAudio, maxModelTokens, maxResponseTokens, proxyUrl, timeout, deductType, deduct, maxRounds, isDraw, front, displaySort, description } = row
  nextTick(() => {
    Object.assign(formPackage, { keyType, appid, modelName, key, secret, canUpload, canAudio, status, model, keyWeight, maxModelTokens, maxResponseTokens, proxyUrl, timeout, deductType, deduct, maxRounds, isDraw, front, displaySort, description })
  })
  visible.value = true
}
  ...
  formEl?.validate(async (valid) => {
    ...
    if (valid) {
      ...
      if(Number(formPackage.keyType) === 1){
        const key = deepClone(formPackage.key)
  ...
  <div>
    <page-main>
      <el-form ref="formRef" :inline="true" :model="formInline">
        <el-form-item label="模型分类" prop="model">
          <el-select v-model="formInline.keyType" filterable allow-create placeholder="请选择或填写绑定的模型" clearable style="width: 192px;">
            <el-option v-for="item in MODELTYPELIST" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="模型类型" prop="modelType">
          <el-select v-model="formInline.modelType" filterable allow-create placeholder="请选择模型类型" clearable style="width: 192px;">
            <el-option v-for="item in ModelTypes" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="使用模型" prop="model">
          <el-select v-model="formInline.model" filterable allow-create placeholder="请选择或填写绑定的模型" clearable style="width: 192px;">
            <el-option v-for="item in MODEL_LIST" :key="item" :label="item" :value="item" />
          </el-select>
        </el-form-item>
        <el-form-item label="启用状态" prop="status">
          <el-select v-model="formInline.status" placeholder="请选择key启用状态" clearable style="width: 192px;">
    ...
    <page-main bottomPadding="20px">
      <el-alert show-icon  :closable="false" title="KEY池说明" description="所有key会根据你所选的模型自动分类、会以第一个加入的key为标准、建议所有同模型的key参数配置相同、否则可能特殊场景会有迷惑性。不同模型可以设置不同的扣费类型以及单次调用扣除的额度、额度不再以次为单位、改为(积分)概念！" type="success" />
      <!-- 你可以设置模型最大上限和回复上限、以及最大轮次用以限制用户的最大上下文、当限制token后、用户端可选范围受其影响、轮次同理、到达轮次之后会移除掉更靠前的上下文！ -->
    </page-main>
    <page-main bottomPadding="20px" bottomMargin="0px">
      <el-table v-loading="loading" height="52vh" border :data="tableData" style="width: 100%;" size="large">
        <el-table-column prop="keyType" label="模型分类" width="120">
        ...
        <el-table-column prop="keyType" label="模型类型" width="120">
          <template #default="scope">
            <el-tag type="success">
              {{ MODELTYPESMAP[scope.row.modelType] }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="model" align="center" label="绑定模型"  width="180">
          <template #default="scope">
            <el-tag :type="scope.row.model.includes('gpt-4') ? 'success' : undefined">
              {{ scope.row.model }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="visible"
      ...
      >
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="模型分类选择" prop="keyType">
          ...
          <el-col :span="12">
            <el-form-item label="模型类型" prop="modelType">
               <el-select v-model="formPackage.modelType" placeholder="请选择模型类型" style="width: 90%;">
                 <el-option
          ...
          <el-col :span="24">
            <el-form-item :label="labelKeyName" prop="key">
              <el-input v-model="formPackage.key" :type="'text'" placeholder="请填写模型Key|clientId|AppId" style="width: 95.7%;"/>
              <el-tooltip
                class="box-item"
                effect="dark"
                placement="right"
          ...
          <el-col :span="24">
            <el-form-item label="SecretKey" prop="secret" v-if="[2, 4].includes(Number(formPackage.keyType))">
              <el-input v-model="formPackage.secret" placeholder="请填写模型调用所需的Secret[部分类型模型需要]"  style="width: 95.7%;"/>
              <el-tooltip
                class="box-item"
                effect="dark"
                placement="right"
          ...
          <!--  讯飞星火需要appid -->
          <el-col :span="24">
            <el-form-item label="AppId" prop="appid" v-if="[4].includes(Number(formPackage.keyType))">
              <el-input v-model="formPackage.appid" placeholder="请填写模型调用所需的appid"  style="width: 95.7%;"/>
              <el-tooltip
                class="box-item"
                effect="dark"
                placement="right"
          ...
          <el-col :span="24">
            <el-form-item label="模型描述" prop="description">
               <el-input type="textarea" v-model.number="formPackage.description" placeholder="请填写模型描述" style="width: 95.7%;" />
             </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="模型扣费类型" prop="deductType">
              <el-select v-model="formPackage.deductType" filterable allow-create clearable placeholder="请选用模型扣费类型" style="width: 90%;">
                <el-option
          ...
          <el-col :span="12">
            <el-form-item label="单词扣除金额" prop="deduct">
              <el-input v-model.number="formPackage.deduct" placeholder="请填写单词调用此key的扣费金额！" style="width: 90%;" />
              <el-tooltip
                class="box-item"
                effect="dark"
                placement="right"
          ...
          <el-col :span="12">
            <el-form-item label="调用轮询权重" prop="keyWeight">
              <el-input v-model.number="formPackage.keyWeight" placeholder="请填写key的权重、数字越大使用评率越高！" style="width: 90%;" />
              <el-tooltip
                class="box-item"
                effect="dark"
                placement="right"
          ...
          <el-col :span="12">
            <el-form-item label="显示排序" prop="displaySort">
              <el-input v-model.number="formPackage.displaySort" placeholder="显示排序" style="width: 90%;" />
              <el-tooltip
                class="box-item"
                effect="dark"
                placement="right"
          ...
          <el-row style="width: 100%;">
            ...
            <el-col :span="6">
              <el-form-item label="前台显示" prop="front">
                <el-switch
                  v-model="formPackage.front"
                  :active-value="true"
                  :inactive-value="false"
                />
                <el-tooltip
        ...
        <div style="display: none;">
          <el-form-item label="设为特殊key" prop="isDraw" v-if="[1].includes(Number(formPackage.keyType))">
            ...
            >
              <template #content>
                <div style="width: 250px;">
                  基础绘画来自于OPENAI的DALL-E2模型、所以需要为官方的apiKey、请确定至少设置一张key为基础绘画key即可使用绘画功能！同时当前版本的mind思维导图和mj联想绘图等功能都会走当前设置的key，会后后续版本解除此限制！
                </div>
              </template>
              <el-icon class="ml-3 cursor-pointer"><QuestionFilled /></el-icon>
            </el-tooltip>
          ...
          <el-form-item label="上下文限制" prop="maxRounds">
            <el-input v-model.number="formPackage.maxRounds" placeholder="请填写允许用户选择的最高上下文轮次！" style="width: 90%;" />
            <el-tooltip
              class="box-item"
              effect="dark"
              placement="right"
