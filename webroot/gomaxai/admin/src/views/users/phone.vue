...
async function queryAllconfig() {
  const res = await apiConfig.queryConfig({ keys: ['phoneRegisterStatus', 'aliPhoneAccessKeyId', 'aliPhoneAccessKeySecret', 'aliPhoneSignName', 'aliPhoneTemplateCode'] })
          ...
          <el-col :xs="24" :md="20" :lg="15" :xl="12">
            <el-form-item label="是否开启手机号注册" prop="phoneRegisterStatus">
              <el-tooltip
                class="box-item"
                effect="dark"
                content="如您启用短信验证、则用户端则可以通过手机验证码方式注册或登录！"
                placement="right"
              >
                <el-switch
