  ...
  () => {
    ...
    function hasPermission(permissions: string[], route: Route.recordMainRaw | RouteRecordRaw) {
      ...
      if (route.children?.length > 0) {
        ...
        const result = permissions.includes(route.name)
    ...
    const routes = computed(() => {
      ...
      console.log('role include',!['super', 'admin'].includes(userStore.userRole));

      if (settingsStore.settings.app.enablePermission && !['super', 'admin'].includes(userStore.userRole)) {
         // removeRoutes();
