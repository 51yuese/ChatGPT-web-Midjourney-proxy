...
export const useGlobalStore = defineStore('global-store', {
    ...
    state: (): GlobalState => ({
        ...
        userInfoDialog: false, //  选择模型弹窗
				pointsDialog:false, // 积分规则对话框
        isChatIn: false, // 是否在对话
        fingerprint: 0, // 是否录音弹窗
				appModelDialog: false, // 应用切换模型弹窗
				// points
    ...
    actions: {
        ...
        updateNoticeDialog(noticeDialog: boolean) {
            this.noticeDialog = noticeDialog
				...
				// 修改积分规则对话框状态
				updatePointsDialog(pointsDialog: boolean){
        ...
        updateModel(model: number) {
            ...
            this.model = model
				...
				updateModelData(model: object){
        	this.modelData = model
				},

				updateAppModelDialog(status: boolean){
        	this.appModelDialog = status;
				}
