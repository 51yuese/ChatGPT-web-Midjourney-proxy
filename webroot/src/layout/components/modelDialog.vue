...
import {fetchQueryModelsListAPI,fetchQueryModelsListAPIMaps} from '@/api/models'
...
async function queryModelsList() {
	...
	try {
		...
		const res: any = await fetchQueryModelsListAPIMaps(data)
		const res1: any = await fetchQueryModelsListAPI()
...
async function handleUpdateConfig() {
	...
	const {canAudio,canUpload,keyType,modelName,model,deductType,deduct}=modelInfo.value
	const config = {
		modelInfo: {
			canAudio,
			canUpload,
		...
		modelTypeInfo: getModelTypeInfo(keyType)
	}
...
const modelInfo=ref()
// 监听对话模型改变事件
function changModelItemClick(item) {
	selectModel.value = item.modelName
...
<template>
	...
	<NModal :show="visible" style="width: 50%;" :on-after-enter="openDialog"
					:on-after-leave="handleCloseDialog">
		...
		<div class="py-3 px-5  rounded dark:bg-[#15171A] bg-white">
			...
			<div class="model_list">
				<div class="bg-[#F5F8FE] dark:bg-[#202B3A]" :class=" selectModel == item.modelName? 'model_item':'def_model_item'"
						 v-for="(item,index) in options" :key="item.id" @click="changModelItemClick(item)">
					<div class="model_name text-[#808080] dark:text-[#fff]">
						<div style="display: flex;width: 70%">
							<img class="model_name_img" v-if="selectModel == item.modelName" src="../../assets/images/selectRound.png">
...
<!--						使用最多的模型，能够进行广泛领域的自然对话和问题解答开通会员 无限使用-->
						{{item.description}}
					</div>
				</div>
			</div>
