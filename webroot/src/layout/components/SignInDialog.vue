...
<template>
  <NModal class="dark:bg-[#15171A] bg-[#fff]" :show="visible" :style="{ maxWidth: '650px', minWidth: isSmallLg ? '100%' : '650px' }" :on-after-enter="openDrawerAfter" :on-after-leave="handleCloseDialog">
    <NSpace vertical>
      <NCard closable @close="handleClose">
        <template #header>
          <span class="text-base">签到奖励 <span>（已连续签到<b class="text-[red]">{{ consecutiveDays }}</b>天）</span></span>
				...
				<div class="bg-[#f4f4f4] dark:bg-[#010102]">
					<NAlert style="background-color: transparent" class="mb-5 p-0 " :show-icon="false" type="primary">
						每日签到赠送：
