...
async function queryNotice() {
	const res: ResData = await fetchGetMemberAgreementAPI({keys: ['memberAgreementTitle', 'memberAgreement']})
		...
		<NCard :header-style="{'text-align': 'center'}" :content-style="{'text-align': 'justify'}">
			<template #header>
				...
				<template v-else :class="'m-auto text-center'">
					<span class="text-xl">{{ notice.memberAgreementTitle }}</span>
				</template>
			</template>
			<div v-if="loading" class="px-[20px]">
			...
			<template v-else>
				<div :style="{ background: theme === 'dark' ? '#2c2c32' : '#fff' }"
						 class="p-[20px] markdown-body markdown-body-generate  overflow-y-auto overflow-x-hidden">
					{{ notice.memberAgreement }}
				</div>
