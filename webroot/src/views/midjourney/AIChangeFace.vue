...
import { fetchGetDraw, fetchMidjourneyDrawList } from "@/api/mjDraw";
...
import { NSpace, NSlider, NSwitch, NUpload, useMessage, NUploadDragger } from 'naive-ui';
...
const handleChangeUploadMixImage = (ev: { file: { file: File, status: string; type: string; id: string; }; }) => {
	if (ev.file.status === 'pending') {
		fileToBase64(ev.file.file).then(res => {
			...
			base64Array.value=base64Str
			EventBus.emit("sourceBase64", base64Array.value)
...
const handleBeforeUploadMj1 = (ev: { file: { name: string, type: string }; fileList: FileList[] }) => {
...
const handleChangeUploadMixImage1 = (ev: { file: { file: File, status: string; type: string; id: string; }; }) => {
	if (ev.file.status === 'pending') {
		fileToBase64(ev.file.file).then(res => {
			let base64Str = 'data:image/png;base64,' + res
			base64Array.value=base64Str
			EventBus.emit("targetBase64", base64Array.value)
...
// const removeImg = (ev: { file: { name: string; id: string; status: string; type: string; url: string; }; }) => {
// 	const name = ev.file.name;
...
const ModIdx = ref(0)
const AiFaceMode=ref(1)//1==快图模式,2==慢速模式
function handleMode(idx) {
	...
	EventBus.emit("AiFaceMode",AiFaceMode.value)
}
		...
		<div style="width: 100%;">
			...
			<div class="h-full flex justify-start items-between" style="height: 100vh;width: 100%;">
				<div class="contentBox dark:bg-[#15171A] bg-[#FFFFFF]">
					...
					<div class="first">第一步，上传人脸模板图片</div>
					<div class="padMapBox">
						<div class="intro text-[#FF837E] dark:text-[#FF6C60]">严禁上传违规图(政治人物、色情、暴恐等)违者100%封号</div>
						...
						<div class="intro text-[#A3A5A6] dark:text-[#76777A]">注意素材质量，保证脸部足够清晰，为了更好的效果，最好正脸。</div>
					</div>
					<div class="second">第二步，上传需要替换人脸的目标图</div>
					<div class="padMapBox">
						<div class="intro text-[#FF837E] dark:text-[#FF6C60]">严禁上传违规图(政治人物、色情、暴恐等)违者100%封号</div>
						<div class="modePutExpand">
							<div class="upLoadContent">
								<div>
									<n-upload accept=".jpg,.jpeg,.png" action="" :default-upload="false"
										:on-change="handleChangeUploadMixImage1" :on-before-upload="handleBeforeUploadMj1"
										:on-remove="handleRemoveImageToText1" list-type="image-card" directory-dnd>
									</n-upload>
								</div>
							</div>
						</div>
						<div class="intro text-[#A3A5A6] dark:text-[#76777A]">最大2048x2048像素，不超过5MB，建议：五官无遮挡，角度小于20%</div>
						<div class="intro text-[#FF837E] dark:text-[#FF6C60]" style="margin-top: 4px;">换脸操作只能在写实版人物上进行，漫回类因面部五官比例不统一，是无法完成换脸操作!</div>
					</div>
					<div class="third">第三步，提交任务</div>
					<div class="thirdIntro dark:text-[#F4F4F4] text-[#03060C]">免责声明： 本功能效果太逼真！请遵守相关法律法规，反对非法用途，包括但不限于侵犯他人隐私、散布虚假信息，进行诈骗等行为！</div>
				...
				<div class="contentCenter bg-[#fff] dark:bg-[#15171A]">
					<div
						class="contentCenterTop  border-b-[2px]  bg-[#fff] dark:bg-[#15171A] border-b-[2px] dark:border-[#000000] border-[#D1D1D1]">
						<span># &nbsp;创作中心</span>
						<span
