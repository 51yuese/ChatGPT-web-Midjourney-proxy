...
import {
	...
	fetchQueryGptsByPageAPI, fetchQueryMineAppsAPI
} from "@/api/appStore";
...
// 全部的索引值
const AppAllIndex = computed(() => authStore.AppAllIndex)
// 应用列表索引值
...
async function getAppList() {
	if (selectSiderIndex.value == AppAllIndex.value){
		// 当前为全部
		...
		// 根据分类id查询应用数据
		const res = await fetchQueryGptsByPageAPI({page: 0,size: 0,modelName: 0,	modelId: '',groupId: selectSiderIndex.value});
		if (res.code == 200) {
			appList.value = res.data.rows
		}
	}
			...
			<div style="display: flex;overflow: hidden">
				...
				<div style="width: 55%">
					<div style="display: flex;align-items: center;justify-content: space-between;">
						...
						<svg style="min-height: 20px;min-width: 20px;position: absolute;right: 20px;" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" v-if="item.collected == true" @click="collectApp(item.id)">
