...
import {useAppStore, useAuthStore, useGptsStore} from '@/store'
...
const dataSources = computed(() => gptsStore.groupList)
const groupKeyWord = computed(() => gptsStore.groupKeyWord)
...
async function handleSelect(group: Chat.History) {
  ...
  await gptsStore.setActiveGroup(uuid)
  // if (route.name !== 'Chat')
  //   router.push('/chat')
  //
  // if (isMobile.value)
...
async function handleDelete(params: Chat.History) {
  ...
  await gptsStore.deleteGroup(params)
