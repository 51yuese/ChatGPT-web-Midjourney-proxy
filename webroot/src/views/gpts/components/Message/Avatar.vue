...
const robotAvatar = computed(() => authStore.globalConfig.robotAvatar)
</script>

<template>
  <template v-if="image">
    <NAvatar style="width: 60px;height: 60px;border-radius: 12px;margin-right: 0.5rem" v-if="isString(avatar) && avatar.length > 0" :size="isMobile ? 32 : 42" :src="avatar" :fallback-src="defaultAvatar" />
    <NAvatar style="width: 60px;height: 60px;border-radius: 12px;margin-right: 0.5rem" v-else :size="isMobile ? 32 : 42" :src="defaultAvatar" />
  </template>
  <span v-else class="text-[28px] dark:text-white">
    <NAvatar  v-if="isString(robotAvatar) && robotAvatar.length > 0" style="background-color: transparent;width: 60px;height: 60px;border-radius: 12px" :size="isMobile ? 32 : 32" :src="groupInfo.logo" :fallback-src="defaultAvatar" />
