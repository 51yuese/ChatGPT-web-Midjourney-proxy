...
function http<T = any>(
  { url, data, method, headers, onDownloadProgress, onUploadProgress, signal, beforeRequest, afterRequest }: HttpOption,
) {
  ...
  return method === 'GET'
    ...
    : request.post(url, params, { headers, signal, onDownloadProgress, onUploadProgress }).then(successHandler, failHandler)
}
...
export function post<T = any>(
  { url, data, method = 'POST', headers, onDownloadProgress, signal, beforeRequest, afterRequest, onUploadProgress, timeout }: HttpOption,
): Promise<Response<T>> {
  return http<T>({
    url,
    method,
