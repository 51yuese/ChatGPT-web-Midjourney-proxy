...
import {fetchQueryMenuAPI, setThemeAPI} from '@/api/config'
...
<template>
  <div class="flex min-w-[80px]  bg-[#E7ECF6] pb-2 dark:bg-[#25272d]  dark:border-[#3a3a40]    border-#efeff5-800 " :class="getMobileLayoutClass" :style="mobileSafeArea">
    ...
    <main :class="['flex-1 flex-grow-1 mb-5 overflow-auto', getIconMobileLayoutClass]" ref="track">
      ...
      <div class="flex h-full flex-col items-center space-y-4">
        <div v-for="item in menuList" :key="item.menuName" class="inline-block" :class="isMobile ? 'mt-0' : 'mt-3'" @click="handleClickMenu(item)">
          <NTooltip trigger="hover" placement="right">
            <template #trigger>
							<!-- dark:bg-[#191a1a] -->
              <div class="h-12 w-12 cursor-pointer rounded-xl  bg-white duration-300 dark:bg-[#3B3B3B] " :class="[item.menuPath === activeRoutePath || iframeSrc === item.menuIframeUrl ? 'shadow-[#3076fd] btn' : '']">
                <div class="flex h-full justify-center">
                  <div class="m-auto text-center">
                    <SvgIcon :icon="item.menuIcon" class="text-2xl hover:scale-125 transition-all text-center" :class="[item.menuPath === activeRoutePath || iframeSrc === item.menuIframeUrl ? 'text-[#3076fd] dark:text-[#3076fd] ' : '']" />
                 		<p  style="line-height: 1.2; font-size: 0.6rem">{{item.menuName}}</p>
                  </div>
                </div>
              </div>
            </template>
